...
Addr YagsBP::extTageAddr(Addr addr){
        return addr & cacheTagMask;
}
YagsBP::YagsBP(const YagsBPParams &params)
    : BPredUnit(params), globalHistoryReg(params.numThreads, 0),
      ...
      tagCacheBits(params.tagCacheBits),
      tagCacheSize(params.tagCacheSize),
...
void
YagsBP::updateHistories(ThreadID tid, Addr pc, bool uncond,
...
bool YagsBP::lookupTageCache(std::vector<CacheEntry>& cache,
                      ...
                      unsigned addr, Addr tag, bool &hit){
        hit = false;
...
YagsBP::lookup(ThreadID tid, Addr branchAddr, void * &bp_history)
{
    ...
    notTakenCachePreiction =
        lookupTageCache(notTakenCache,notTakenCacheCounters, globalHistoryIdx, branchAddr, notTakenCacheHit);
    takenCachePreiction =
        lookupTageCache(takenCache, takenCacheCounters, globalHistoryIdx, branchAddr,takenCacheHit);
                 ...
                 bool squashed, const StaticInstPtr & inst, Addr target)
{
    ...
    if(insertTakenCache) {
            ...
            notTakenCacheCounters[globalHistoryIdx] = SatCounter8(tagCacheBits, choiceThreshold + 1);
    }

    if(insertNotTakenCache) {
            takenCache[globalHistoryIdx].invalidate();
            takenCache[globalHistoryIdx].insert(branchAddr);
            notTakenCacheCounters[globalHistoryIdx]= SatCounter8(tagCacheBits, choiceThreshold);
    }
